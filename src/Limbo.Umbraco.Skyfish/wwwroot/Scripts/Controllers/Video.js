angular.module("umbraco").controller("Limbo.Umbraco.Skyfish.Video", function ($scope, $element, $timeout, skyfishService) {
    ...
    vm.getVideo = function () {
        ...
        if (source) {
            ...
            skyfishService.getVideo(source).then(function (res) {
                ...
                vm.value.details = { _data: angular.toJson(res.data.details) };
                vm.value.embed = res.data.embed;
                rawVideoData = res.data.details;
    ...
    vm.updateUI = function () {
        ...
        vm.id = rawVideoData.uniqueMediaId;
        vm.folderId = rawVideoData.folderId;
        vm.title = rawVideoData.title ? rawVideoData.title : rawVideoData.fileName;
        vm.description = rawVideoData.description;
        vm.thumbnail = skyfishService.getThumbnail(rawVideoData);
        vm.appUrl = vm.folderId ? `https://app.skyfish.com/folder/${vm.folderId}/file/${vm.id}` : null;
    ...
    function init() {
        ...
        rawVideoData = angular.fromJson($scope.model.value.details._data);
